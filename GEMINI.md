...
1. **App Version**: The current app version is `0.1.1`. The app version must ONLY be raised after explicit user confirmation. The app version takes the lead on major version increments.
...
3. **Version Alignment**: The major (first) digit of the App Version and File Format Version must always match (e.g., App `0.1.1` maps to File Format `0.1`). When the App Version reaches `1.0.0`, the File Format Version must advance to `1.0`.
