...
class NetworkSolver:
    ...
    def prune_topology(self):
        ...
        # 3. Reachability Analysis (undirected DFS from pressure boundaries or fluid sources)
        boundary_ids = [
            nid for nid, node in self.network.nodes.items()
            if getattr(node, 'is_pressure_boundary', False) or getattr(node, 'node_type', None) == 'fluid_source'
        ]
        ...
        while True:
            ...
            for nid in reachable_node_ids:
                ...
                is_boundary = getattr(node, 'is_pressure_boundary', False) or getattr(node, 'node_type', None) == 'fluid_source'
    ...
    def calculate_jacobian(self, x_scaled) -> np.ndarray:
        ...
        for a, node_idx in enumerate(self.internal_node_indices):
            ...
            # If a fluid source in flow mode has an unconnected inlet port,
            # its derivative with respect to its own pressure is 1.0, and 0 for everything else.
            if getattr(node, 'node_type', None) == 'fluid_source' and not getattr(node, 'is_pressure_boundary', False):
                inlet_connected = False
                for e in self.edges_list:
                    if (e['target'] == node_id and e.get('target_port') == 'inlet-0') or \
                       (e['source'] == node_id and e.get('source_port') == 'inlet-0'):
    ...
    def _solve_hydraulics_core(self, method='sparse_newton', x0_custom=None) -> Tuple[np.ndarray, int, int, bool, np.ndarray]:
        ...
        def objective(x_scaled):
            ...
            for i, node_idx in enumerate(self.internal_node_indices):
                ...
                # If a fluid source in flow mode has an unconnected inlet port, bypass mass balance
                # and pin its inlet pressure to the ambient/atmospheric pressure instead.
                if getattr(node, 'node_type', None) == 'fluid_source' and not getattr(node, 'is_pressure_boundary', False):
                    inlet_connected = False
                    for e in self.edges_list:
                        if (e['target'] == node_id and e.get('target_port') == 'inlet-0') or \
